-- ===========================================================================
-- LANCAMENTO GRATIS (lado do ANUNCIANTE) — flag reversivel via app_config
-- 'true'  = libera EMPREGO (sem Essencial) E desbloqueio de DIARIA (sem R$2,50)
--           para TODOS os anunciantes.
-- 'false' = cobra tudo normal (comportamento atual).
-- NAO toca no gate do prestador (3 gratis / R$9,90) nem no cap de 10.
-- Recria as 2 pecas vigentes (promo_boas_vindas_30dias) + o desvio da flag.
-- Idempotente (CREATE OR REPLACE). Rodar 1x no Supabase -> SQL Editor -> Run.
-- Rodar isto NAO muda comportamento ate voce fazer UPDATE app_config = 'true'.
-- ===========================================================================
...
-- 2) Helper lido pela RPC, pelo trigger e pelo app
...
-- 3) RPC + 4) Trigger (= versao vigente + desvio da flag, marcado com [NOVO])
...
BEGIN
  ...
  -- ── PROMO BOAS-VINDAS: 30 dias do cadastro = TUDO liberado (diária E emprego) ──
  SELECT created_at INTO v_cadastro_em FROM user_profiles WHERE id = v_uid;
  v_promo_expira := v_cadastro_em + INTERVAL '30 days';
  IF v_cadastro_em IS NOT NULL AND NOW() < v_promo_expira THEN
    RETURN jsonb_build_object(
      ...
      'promo_boas_vindas', true,
      'promo_expira_em',   v_promo_expira
    );
  END IF;

  -- ── LANÇAMENTO GRÁTIS (anunciante): flag global desliga os 2 gates ── [NOVO]
...
BEGIN
  ...
  -- ── LANÇAMENTO GRÁTIS (anunciante): flag global libera a seleção sem cobrança ── [NOVO]
  IF launch_free_anunciante() THEN
    RETURN NEW;
  END IF;
...
SELECT 'launch_free_anunciante instalada (flag OFF por padrao). Ligue com UPDATE app_config.' AS resultado;
