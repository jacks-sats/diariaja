-- ═══════════════════════════════════════════════════════════════════════════
-- PROMO BOAS-VINDAS: novo anunciante = 30 dias de contatos ILIMITADOS grátis
-- ═══════════════════════════════════════════════════════════════════════════
-- Decisão do dono (28/06/2026): TODO novo anunciante ganha, automaticamente, os
-- primeiros 30 dias (a partir do cadastro) com contatos ILIMITADOS de graça —
-- cobre DIÁRIA (sem R$ 2,50) E EMPREGO (sem exigir plano Essencial). Passados os
-- 30 dias, volta a valer a regra normal (3 grátis/mês + R$ 2,50 na diária;
-- Essencial pra emprego).
--
-- Fonte da data: user_profiles.created_at (cadastro do perfil). 1 período por
-- conta → não dá pra farmar (não depende de recusa, nem reseta por mês).
--
-- Recria as DUAS peças (RPC consultiva + trigger de enforcement), adicionando o
-- ramo da promo NO TOPO, antes da lógica de emprego/diária. Mantém todo o resto
-- intacto. Idempotente (CREATE OR REPLACE).
...
SELECT 'Promo boas-vindas (30 dias ilimitado pro novo anunciante) instalada.' AS resultado;
